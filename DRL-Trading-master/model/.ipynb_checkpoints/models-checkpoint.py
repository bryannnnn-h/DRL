...
def DRL_prediction(df,
                   ...
                   initial):   # actual trading process
...
def run_ensemble_strategy(df, unique_trade_date, rebalance_window, validation_window) -> None:
    ...
    for i in range(rebalance_window + validation_window, len(unique_trade_date), rebalance_window):
        ...
        # record sharp ratio for each agent in agent pool
        ...
        '''
            Ensemble adjustment
        '''
        action_weights = [exp(sharpe_a2c), exp(sharpe_ppo), exp(sharpe_ddpg)]   # action weights for ensemble
        weight_base = sum(action_weights)
        model_ensemble = [model_ppo, model_a2c, model_ddpg]   # agent pool for ensemble
        ...
        last_state_ensemble = DRL_prediction(df=df, model=model_ensemble, action_weights=action_weights, weight_base=weight_base, name="ensemble",
