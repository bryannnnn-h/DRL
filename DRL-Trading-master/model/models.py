...
def get_validation_sharpe(iteration,val_data,start,over):
    ...
    over = '-'.join(over_sp)
    for tic in tic_set:
        #print(val_data["datadate"][0].drop_duplicates())
        #exit()
        #startdate = val_data["datadate"][0].drop_duplicates()
        tmp_start = val_data.loc[(val_data["tic"] == tic)&(val_data["datadate"] == start)]["open"][0]
...
def DRL_prediction(df,
                   ...
                   initial,
                   vix, iter_total):   # actual trading process
...
def train_model(model_use,env_train,unique_trade_date,rebalance_window,validation_window,i,validation,env_val,obs_val,valid_start_date,valid_over_date):
    if model_use=='a2c': 
        print("======A2C Training========")
        ...
        sharpe_ddpg = get_validation_sharpe(i,validation,valid_start_date,valid_over_date)
...
def run_ensemble_strategy(df, unique_trade_date, rebalance_window, validation_window,model_use,iter_number) -> None:
    ...
    for i in range(rebalance_window + validation_window, len(unique_trade_date), rebalance_window):
        ...
        model_ensemble,sharpe=train_model(model_use,env_train,unique_trade_date,rebalance_window,validation_window,i,validation,env_val,obs_val,unique_trade_date[i - rebalance_window - validation_window],unique_trade_date[i - rebalance_window])
